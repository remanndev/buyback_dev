...
class User extends CI_Controller
{
	...
	// NPO회원 목록
	public function grplists()
	{
		$this->_lists(20);
	}

	private function _lists($user_level=10)
	{
		...
		if( $this->input->post('excel_truncate_submit') ) {
			...
			if('truncate' == $chk) 
			{
				...
				// 회원 정보를 완전히 삭제
				if($this->tank_auth->delete_excel_user_by_admin('관리자삭제')) {
			...
			if('upload' == $chk) 
			{
				...
				for($i = 0; $i < $sheetsCount; $i++)
				{
					...
					for ($row = 2; $row <= $highestRow; $row++)
					{
						...
						$nickname = isset($udata[0]) ? $udata[0] : FALSE;
						$username = isset($udata[1]) ? $udata[1] : FALSE;
						$password = isset($udata[2]) ? $udata[2] : FALSE;
						$email = isset($udata[3]) ? $udata[3] : FALSE;
						$phone = isset($udata[4]) ? $udata[4] : FALSE;
						//$birth = isset($udata[5]) ? $udata[5] : FALSE;
						$birth = PHPExcel_Style_NumberFormat::toFormattedString($udata[5], PHPExcel_Style_NumberFormat::FORMAT_DATE_YYYYMMDD2);


						$this->tank_auth->excel_user_by_admin($nickname,$username,$password,$email,$phone,$birth);
					}
				}
		...
		// [sql] where option 
		$sql_where_option = array('deleted'=>NULL,'level'=>$user_level);
		...
		// pagination 설정
		// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		$userlist = ($user_level == 20) ? 'grplists' : 'lists';
		$config['suffix']	   = $qstr; //$qstr;
		$config['base_url']    = base_url() . 'admin/user/'.$userlist.'/page/';
		...
		// 엑셀 다운로드
		$btn_excel_download = '<a href="/admin/user/excel_download_member/'.$level.'/'.$sfl.'/'.$stx.'" class="btn btn-dark btn-xs o_btn">엑셀 다운</a>';
		...
		$data = array(
			...
			'viewPage'  => 'admin/user_lists_view'
		);
		...
		* [2017-02-17] 회원가입 에러 메시지 CSS */
		...
		if( $this->input->post('submit') ) {
			...
			// 공통
				...
				// npo 회원용
				$this->form_validation->set_rules('npo_company', '단체명', '');
				$this->form_validation->set_rules('npo_manager', '담당자명', '');
				$this->form_validation->set_rules('npo_tel', '연락처', '');
	...
	function del($user_idx=FALSE,$level='')
	{
		// 회원 정보를 완전히 삭제
		if($this->tank_auth->delete_user_by_admin($user_idx,'관리자삭제')) {
			...
			if($level == 20) {
				redirect(base_url().'admin/user/grplists');
			}
			else {
				redirect(base_url().'admin/user');
			}
	...
	function admlists() 
	{
		...
		// [sql] where option 
		$sql_where_option = array('deleted'=>NULL, 'username !='=>'sadmin', 'type !=' => 'PARTNER');
		...
		* [2017-02-17] 회원가입 에러 메시지 CSS */
		...
		if( $this->input->post('submit') ) {
			...
			if ($this->form_validation->run()) {	// validation ok
				...
				$email    = ($this->form_validation->set_value('user_email')) ? $this->form_validation->set_value('user_email') : '';
				$password = ( $is_set_pw ) ? $this->form_validation->set_value('member_pw') : '';
				$admin_type = strtoupper(trim((string) $this->input->post('admin_type', TRUE)));
				$_POST['admin_type'] = in_array($admin_type, array('SITE', 'BOTH'), TRUE) ? $admin_type : 'SITE';

				$current_admin_username = '';
				if ($user_idx) {
					$current_admin = $this->auth_users->get_admin_by_id($user_idx, TRUE);
					if (is_object($current_admin) && isset($current_admin->username)) {
						$current_admin_username = (string) $current_admin->username;
					}
				}
				if (!$user_idx || !in_array(strtolower($current_admin_username), array('sadmin', 'admin'), TRUE)) {
					$_POST['level'] = '90';
				}
				else {
					unset($_POST['level']);
				...
				if (!is_null($data = $this->tank_auth->write_manager($user_idx,$username,$email,$password))) {		// success
					$saved_admin_id = $user_idx ? (int) $user_idx : (isset($data['user_id']) ? (int) $data['user_id'] : 0);
					if ($saved_admin_id > 0) {
						$update_admin_data = array(
							'type' => $_POST['admin_type'],
						);
						if (!$user_idx || !in_array(strtolower($current_admin_username), array('sadmin', 'admin'), TRUE)) {
							$update_admin_data['level'] = 90;
						}
						$this->db->where('id', $saved_admin_id);
						$this->db->update('users_admin', $update_admin_data);
					}
		...
		$row = $this->basic_model->arr_get_row($arr);
		if ($row && isset($row->type) && strtoupper((string) $row->type) === 'PARTNER') {
			alert('회원사 전용 계정은 회원사 관리 메뉴에서 관리해주세요.', BASEURL.'/admin/user/admlists');
		}
