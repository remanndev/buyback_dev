<?php
/*
// 회원구분( 0:비회원 / 1:일반회원 / 2:NPO회원·sns로그인회원)
$arr_user_type = array(
	1=>'일반회원', // 일반회원·sns로그인회원
	2=>'NPO회원', // 일반회원·sns로그인회원
);
...
*/
	...
	$selected_level =  isset($row->level) ? $row->level : set_value('level');

	$btn_user_list_link = (isset($row->level) && $row->level == '20') ? '/admin/user/grplists/' : '/admin/user/';
...
<div class=" admin_wrap">
	...
	<h1>회원 <?php echo (isset($user_idx) && $user_idx > 0) ? '정보 수정' : '신규 등록'; ?></h1>
	<h2>기본정보</h2>
	...
	<div class="tbl_frm">
		...
		<!-- <input type="hidden" name="group_fk" value="<?php echo $row->group_fk ?>" /> -->
		<?php //} else { ?>
		<tr>
		  <th>회원 구분</th>
		  <td>
			<?php echo form_dropdown('level', $level_select_options, $selected_level, 'class="o_selectbox"'); ?>
		  </td>
		  <th>가입일자 / 최종로그인(횟수)</th>
		  <td>
			<?php echo (isset($row->created) ? $row->created : ''); ?>  /  <?php echo (isset($row->last_login) ? $row->last_login : ''); ?> <?php echo (isset($row->cnt_login) ? ' ( 총 '. $row->cnt_login .' 회 로그인 )' : ''); ?>
		  </td>
		</tr>
		<?php //} ?>

		<?php //if(isset($user_idx) && $user_idx > 0) { ?>
		<tr>
		  ...
		  <td>
			...
			?>
				...
				<button type="button" class="btn btn-dark btn-xs   check_username" >중복 체크</button>
			...
			<?php } else { echo '아이디 로그인은 사용하지 않습니다.'; } ?>
			<?php } ?>
		  </td>
		  <th>휴대전화</th>
		...
		<tr>
		  <th>이름</th>
		  <td>
			<input type="text" id="nickname" name="nickname" value="<?php echo set_value('nickname', (isset($row->nickname) ? $row->nickname : '' )); ?>" class="o_input" />
		  ...
		  <td>
			...
			<input type="email" id="user_email" name="user_email" value="<?php echo set_value('user_email', (isset($row->email) ? $row->email : '')); ?>" class="o_input" style="width:295px;" />
			...
			<button type="button" class="btn btn-dark btn-xs   check_email" >중복 체크</button>
		...
		// NPO 회원
		// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		if( isset($row->level) && $row->level == 20 ) { ?>
		<tr>
		  <th>NPO 단체명</th>
		  <td>
			<input type="text" id="npo_company" name="npo_company" value="<?php echo set_value('npo_company', (isset($row->company) ? $row->company : '' )); ?>" class="o_input" />
			<?php echo form_error('npo_company','<div class="err_color_red">','</div>'); ?>
		  </td>
		  <th></th>
		  <td>
		  </td>
		...
		<tr>
		  <th>NPO 담당자명</th>
		  <td>
			<input type="text" id="npo_manager" name="npo_manager" value="<?php echo set_value('npo_manager', (isset($row->manager) ? $row->manager : '' )); ?>" class="o_input" />
			<?php echo form_error('npo_manager','<div class="err_color_red">','</div>'); ?>
		  </td>
		  <th>NPO 연락처</th>
		  <td>
			<input type="text" id="npo_tel" name="npo_tel" value="<?php echo set_value('npo_tel', (isset($row->tel) ? $row->tel : '' )); ?>" class="o_input" />
			<?php echo form_error('npo_tel','<div class="err_color_red">','</div>'); ?>
		  </td>
		</tr>
		<?php } ?>
		...
		<tr>
		  ...
		  <td>
			...
			<?php //echo (isset($row->activated) && $row->activated) ? '승인완료' : '미승인상태'; ?>
			...
			<?php 
				if(! isset($row->activated)) {
					...
					echo (isset($row->activated) && $row->activated) ? '승인완료' : '미승인상태';
				}
			?>

			<?php // if(! (isset($row->activated) && $row->activated)) { ?>
			<?php if( isset($row->activated) && $row->activated < 1 ) { ?>
				<button type="button" class="btn btn-xs btn-dark   btn_activate" data-idx="<?php echo $user_idx ?>">승인하기</button>
			<?php } ?>
		  </td>
		  <th>비밀번호 변경</th>
		  <td>
			...
			<input type="password" id="member_pw" name="member_pw" value="" placeholder="비밀번호" class="o_input" />
			<input type="password" id="member_pw_confirm" name="member_pw_confirm" value="" placeholder="비밀번호 확인" class="o_input" />
		...
		<tr>
		  ...
		  <th>차단(ban) 사유</th>
		  <td>
			<input type="text" name="ban_reason" value="<?php echo set_value('ban_reason', (isset($row->ban_reason) ? $row->ban_reason : '')); ?>" class="o_input" style="width:100%; max-width:600px;" />
		  </td>
		</tr>
		<?php } ?>
		...
		<tr>
		  <th>휴대폰번호</th>
		  <td>
			<input type="text" name="upro_phone_1" value="<?php echo set_value('upro_phone_1', (isset($arr_phone[0]) && ! empty($arr_phone[0])) ? $arr_phone[0] : '010'); ?>" maxlength="3" class="o_input" style="width:50px; text-align:center; " />
			-
			<input type="text" name="upro_phone_2" value="<?php echo set_value('upro_phone_2', (isset($arr_phone[1]) && ! empty($arr_phone[1])) ? $arr_phone[1] : ''); ?>" maxlength="4" class="o_input" style="width:70px; text-align:center; " />
			-
			<input type="text" name="upro_phone_3" value="<?php echo set_value('upro_phone_3', (isset($arr_phone[2]) && ! empty($arr_phone[2])) ? $arr_phone[2] : ''); ?>" maxlength="4" class="o_input" style="width:70px; text-align:center; " />
		  </td>
		  <th><!-- 유선 전화번호 --></th>
		  <td>
		...
		*/ ?>
		...
		<tr>
		  <th>주소</th>
		  <td colspan="3">
			<input type="text" id="postcode" name="postcode" value="<?php echo set_value('postcode', (isset($row->postcode) ? $row->postcode : '')); ?>" readonly class="o_input" style="width:100px;text-align:center;" placeholder="우편번호" /> <button type="button" class="btn btn-dark btn-xs " style="vertical-align:middle;" onclick="srh_execDaumPostcode(); return false;" >우편번호 찾기</button>
	...
	<div style="text-align:center; padding:40px 0 20px;">
		...
		<a href="<?php echo $btn_user_list_link ?>" style="text-decoration:none;"><button type="button" class="btn btn-dark btn-sm">목록</button></a>
	</div>
...
$(document).ready(function(){
	...
	$('.btn_activate').on('click', function(){
		...
		if( confirm('가입을 승인하시겠습니까?') ) 
		{
			...
			var uidx = $(this).data('idx');
			var uidx2 = '<?php echo $user_idx ?>';

			
			var request = $.ajax({
			  url: "/admin/user/proc_activate/",
			  method: "POST",
			  data: { 'uidx': uidx, 'uidx2' : uidx2  /* , '<?php echo $this->security->get_csrf_token_name() ?>':'<?php echo $this->security->get_csrf_hash() ?>' */},
			  dataType: "html"
			});
