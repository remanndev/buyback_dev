...
<div class=" admin_wrap">
	...
	?>
		...
		<h2>
			<span style="display:inline-block; width:100px;">회원 목록</span>
		</h2>
		<div>
			<small>
	...
	<div class="tbl_basic">
		...
		<tr>
		  ...
		  <th>구분</th>
		  <th>이름</th>
		  <?php if(isset($this->arr_seg[3]) && 'grplists' == $this->arr_seg[3]) { ?>
		  <th>단체명</th>
		  <?php } ?>
		  <!-- <th>생년월일</th> -->
		  <th>휴대전화</th>
		...
		foreach($list['qry'] as $i => $o)
		{
			...
			if($level > 20) {
				$level_str = '<strong>'.$arr_level[$level].'</strong>';
			}
			else {
				$level_str = $arr_level[$level];
			}
		...
		<tr>
		  ...
		  <td><?php echo $level_str ?></td>
		  <td>
			<?php echo $o->nickname ?>
			<?php //echo (isset($o->manager) && '' != $o->manager) ? ' <span style="font-weight: bold;">('.$o->manager.')</span>' : ''; ?>
			<?php //echo (isset($o->tel) && '' != $o->tel) ? '<div style="font-weight: bold;">'.$o->tel.'</div>' : ''; ?>
		  </td>
		  <?php if(isset($this->arr_seg[3]) && 'grplists' == $this->arr_seg[3]) { ?>
		  <td>
			<?php echo $o->company ?>
		  ...
		  <?php } ?>
		  <!-- <td><?php //echo $o->birth ?></td> -->
		  <td><?php echo $o->phone ?></td>
		  <td><?php echo $o->email ?></td>
		  <!-- <td><?php //echo $o->username ?></td> -->
		  <td><?php echo $o->sns ?></td>
		  <td><?php echo substr($o->created,0,10) ?></td>
		  <td><?php echo substr($o->last_login,0,10) ?></td>
		  <td class='text-center'><?php echo ($o->activated) ? '승인' : '미승인' ?></td>
		  <!-- <td style="width:200px;"><?php //echo (NULL != $o->sleep_email) ? $o->sleep_email : '' ?></td> -->
		  <td class="text-center">
			<a href="/admin/user/write/<?php echo $o->user_idx ?>"><button class="btn btn-secondary btn-xs">수정</button></a>
			<?php if($this->tank_auth->is_admin()) { ?>
			<a href="/admin/user/del/<?php echo $o->user_idx ?>" onclick="del_confirm('admin/user/del/<?php echo $o->user_idx ?>/<?php echo $user_level ?>'); return false;"><button class="btn btn-danger btn-xs">삭제</button></a>
			<?php } ?>
		  </td>
		</tr>
