<?php
// 회원구분( 0:비회원 / 1:준회원 / 2:정회원·sns로그인회원)
$arr_user_type = array(
	1=>'준회원',
	2=>'정회원', // 일반회원·sns로그인회원
);
...
foreach($gr_result['qry'] as $i => $o) :
  $gr_select_options[$o->gr_code] = $o->gr_code.' / '.$o->gr_title;
endforeach;
...
<div class="admin_wrap">
	...
	<h1 style="margin-bottom:0;">
		...
		<h3 class="page-header scroll_fixed" style="z-index:110; width:100%; position:relative;">
		  <div style="position:absolute; bottom:10px; right:0px;">
			<input type="submit" name="submit" class="btn btn-danger btn-sm" value="저장" style="padding-left:15px; padding-right:15px; margin-right:5px;" /><a href="/admin/board/lists"><input type="button" name="list" class="btn btn-dark btn-sm" value="목록" style="padding-left:15px; padding-right:15px; margin-left:0;" /></a>
		  </div>
		</h3>
	</h1>
	...
	<div class="tbl_frm">
		...
		<tr>
		  ...
		  <td>
			...
			<input type="text" id="bo_new_condition" class="o_input" name="bo_new_condition" value="<?php echo set_value('bo_new_condition', $row->bo_new_condition) ?>" style="width:40px; text-align:center;"  /> 시간 이내에 작성된 글
		  </td>
		</tr>
		<tr>
		  <th>HOT 아이콘 설정</th>
		  <td>
			...
			<input type="text" id="bo_hot_condition" class="o_input" name="bo_hot_condition" value="<?php echo set_value('bo_hot_condition', $row->bo_hot_condition) ?>" style="width:40px; text-align:center;"  /> 회 이상 조회된 글
		  </td>
		</tr>
		<tr>
		  <th>관리자 체크</th>
	...
	<div class="tbl_frm">
		...
		<tr>
		  <th>목록 권한</th>
		  <td>
			<label for="bo_level_list_0" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_list_0" name="bo_level_list" value="0" <?php echo  set_radio('bo_level_list', '0', ('0' == $row->bo_level_list) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> 전체(회원+비회원)</label>
			<?php foreach($arr_user_type as $i => $val) { ?>
				<label for="bo_level_list_<?php echo $i ?>" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_list_<?php echo $i ?>" name="bo_level_list" value="<?php echo $i ?>" <?php echo  set_radio('bo_level_list', $i, ($i == $row->bo_level_list) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> <?php echo $val ?></label>
			<?php } ?>
		  </td>
		</tr>
		<tr>
		  <th>보기 권한</th>
		  <td>
			<label for="bo_level_read_0" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_read_0" name="bo_level_read" value="0" <?php echo  set_radio('bo_level_read', '0', ('0' == $row->bo_level_read) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> 전체(회원+비회원)</label>
			<?php foreach($arr_user_type as $i => $val) { ?>
				<label for="bo_level_read_<?php echo $i ?>" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_read_<?php echo $i ?>" name="bo_level_read" value="<?php echo $i ?>" <?php echo  set_radio('bo_level_read', $i, ($i == $row->bo_level_read) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> <?php echo $val ?></label>
			<?php } ?>
		  </td>
		</tr>
		<tr>
		  <th>쓰기 권한</th>
		  <td>
			<label for="bo_level_write_0" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_write_0" name="bo_level_write" value="0" <?php echo  set_radio('bo_level_write', '0', ('0' == $row->bo_level_write) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> 전체(회원+비회원)</label>
			<?php foreach($arr_user_type as $i => $val) { ?>
				<label for="bo_level_write_<?php echo $i ?>" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_write_<?php echo $i ?>" name="bo_level_write" value="<?php echo $i ?>" <?php echo  set_radio('bo_level_write', $i, ($i == $row->bo_level_write) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> <?php echo $val ?></label>
			<?php } ?>
		  </td>
		</tr>
		...
		<tr>
		  <th>답글 설정 및 권한</th>
		  <td>
			<label for="bo_level_reply_0" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_reply_0" name="bo_level_reply" value="0" <?php echo  set_radio('bo_level_reply', '0', ('0' == $row->bo_level_reply) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> 전체(회원+비회원)</label>
			<?php foreach($arr_user_type as $i => $val) { ?>
				<label for="bo_level_reply_<?php echo $i ?>" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_reply_<?php echo $i ?>" name="bo_level_reply" value="<?php echo $i ?>" <?php echo  set_radio('bo_level_reply', $i, ($i == $row->bo_level_reply) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> <?php echo $val ?></label>
			<?php } ?>
		...
		<!-- <tr>
		  <th>코멘트 설정 및 권한</th>
		  <td>
			<label for="bo_level_comment_1" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_comment_1" name="bo_level_comment" value="1" <?php echo  set_radio('bo_level_comment', '1', ('1' === $row->bo_level_comment) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> 전체(회원+비회원)</label>
			<?php foreach($arr_user_type as $i => $val) { ?>
				<label for="bo_level_comment_<?php echo $i ?>" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_comment_<?php echo $i ?>" name="bo_level_comment" value="<?php echo $i ?>" <?php echo  set_radio('bo_level_comment', $i, ($i == $row->bo_level_comment) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> <?php echo $val ?></label>
			<?php } ?>
		...
		<tr>
		  <th>업로드 권한</th>
		  <td>
			<label for="bo_level_upload_1" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_upload_1" name="bo_level_upload" value="1" <?php echo  set_radio('bo_level_upload', '1', ('1' === $row->bo_level_upload) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> 전체(회원+비회원)</label>
			<?php foreach($arr_user_type as $i => $val) { ?>
				<label for="bo_level_upload_<?php echo $i ?>" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_upload_<?php echo $i ?>" name="bo_level_upload" value="<?php echo $i ?>" <?php echo  set_radio('bo_level_upload', $i, ($i == $row->bo_level_upload) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> <?php echo $val ?></label>
			<?php } ?>
		  </td>
		</tr>
		<tr>
		  <th>다운로드 권한</th>
		  <td>
			<label for="bo_level_download_1" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_download_1" name="bo_level_download" value="1" <?php echo  set_radio('bo_level_download', '1', ('1' === $row->bo_level_download) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> 전체(회원+비회원)</label>
			<?php foreach($arr_user_type as $i => $val) { ?>
				<label for="bo_level_download_<?php echo $i ?>" style="cursor:pointer; padding-right:15px;"><input type="radio" id="bo_level_download_<?php echo $i ?>" name="bo_level_download" value="<?php echo $i ?>" <?php echo  set_radio('bo_level_download', $i, ($i == $row->bo_level_download) ? TRUE : FALSE); ?> style="vertical-align:middle;" /> <?php echo $val ?></label>
			<?php } ?>
		  </td>
		</tr> -->
		</table>
	...
	<div class="tbl_frm">
		...
		<!-- <tr>
		  <th>공지 체크 글 설정</th>
		  <td>
			<label for="bo_notice_limit" style="cursor:pointer; padding-right:15px;"><input type="text" id="bo_notice_limit" class="o_input" name="bo_notice_limit" value="<?php echo  set_value('bo_notice_limit', $row->bo_notice_limit); ?>" style="width:30px; text-align:center;" /> 개 노출</label>
		...
		<tr>
		  <th>페이지당 게시물 수</th>
		  <td>
			<input type="text" id="bo_page_limit" class="o_input" name="bo_page_limit" value="<?php echo  set_value('bo_page_limit', $row->bo_page_limit); ?>" style="width:30px; text-align:center;" />
		  </td>
		</tr>
		</table>
	</div>
	...
	<div class="tbl_frm">
		...
		<tr>
		  ...
		  <td>
			...
			<label for="bo_upload_cnt" style="cursor:pointer; padding:0 10px;"> 업로드 수 : <input type="text" id="bo_upload_cnt" class="o_input" name="bo_upload_cnt" value="<?php echo  set_value('bo_upload_cnt', $row->bo_upload_cnt); ?>" style="width:30px; text-align:center;" /> 개</label>
		  </td>
		</tr>
		<tr>
		  <th>첨부 파일 크기 제한</th>
		  <td>
			<label for="bo_upload_size" style="cursor:pointer;"><input type="text" id="bo_upload_size" class="o_input" name="bo_upload_size" value="<?php echo  set_value('bo_upload_size', $row->bo_upload_size); ?>" style="width:30px; text-align:center;" /> MB</label>
			<div class="notice_info_red" style="margin-top:5px; line-height:23px;">
				<strong>에디터에서의 파일 및 이미지 업로드</strong>와 별도의 <strong>첨부 파일 업로드</strong>에 동일하게 적용됩니다.
				<div>서버에서 제한한 용량 이상으로는 설정할 수 없습니다.</div>
			</div>
		  </td>
		</tr>
		<!-- <tr>
