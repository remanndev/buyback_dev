		...
		<dd>
		  <div class="mt_10 py_20 px_20" style="background-color:#f7f7fb; width: 100%; max-width: 980px;">
			...
			<?php if($dn_row->del_date && '' != $dn_row->del_date) { ?>
				...
				<a href="<?php echo base_url('admin/campaign/donation/report_del_ros/'.$cmp_row->code.'/'.$dn_row->idx) ?>" target="_blank"><button type="button" class="o_btn btn btn-purple-line mt_10">데이터 완전 삭제 리포트</button></a>
...
<div class="mt_30">
	<div style="width: 100%; max-width: 980px; text-align: right;">
		<a href="#" onclick="confirm_cancel(this,'<?php echo $cmp_row->code ?>','<?php echo $dn_row->idx ?>'); return false;"  data-list-url="<?php echo base_url('admin/campaign/donation/dn_list/'.$cmp_row->idx); ?>"><button class="btn btn-warning-flat px_20">기부 취소</button></a>
	</div>
</div>
		...
		request.done(function( res ) {
			...
			if( ! res || res != dn_idx ) {
				...
				if('1' == chked_val) {
					...
					var cmp_title = '<?php echo $cmp_row->cmp_title; ?>';
					var request = $.ajax({
					  url: "/admin/campaign/trans/alimtalk_dn_complete",
					  method: "POST",
					  data: { 'dn_idx':dn_idx,'donor_name':donor_name,'donor_phone':donor_phone,'cmp_title':cmp_title },
					  dataType: "html"
					});
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	function confirm_cancel(obj,cmp_code,dn_idx) {
		...
		// data-list-url → obj.dataset.listUrl 로 접근
		const listUrl = obj.dataset.listUrl;

		if(confirm("\n기부자의 취소 의사를 확인하셨습니까?")) {
			if(confirm("정말 취소하시겠습니까?")) {
				var request = $.ajax({
				  url: "/admin/campaign/trans_donation_donor_cancel/",
				  method: "POST",
				  data: { 'cmp_code' : cmp_code, 'dn_idx' : dn_idx},
				  dataType: "text"
				});
