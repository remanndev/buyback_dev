...
class Sns extends CI_Controller {
	function __construct() {
		...
		*/
		...
		// 참고 : https://developers.kakao.com/docs/latest/ko/kakaologin/rest-api
			...
			$this->key_restapi = '67a71d9b8e5f6bbe2eeea364f4c7cc93';
			//$this->redirect_uri = 'http://'.$_SERVER['HTTP_HOST'].'/sns/kakaoLogin';
			$this->redirect_uri = base_url('sns/kakaoLogin');
			$this->client_secret = 'RZLEwkZWaGnqE4Eru0RDwN34KKk0JX77';

			/*
				네이티브 앱 키	93b4ed5a66d1888ef68eb93f825d64d1
				REST API 키	67a71d9b8e5f6bbe2eeea364f4c7cc93
				JavaScript 키	a217aa850a224a735be93a6fbe593173
	...
	*/
	...
	function kakaoLogin() {
		...
		//$shell_string = "curl -v -X POST https://kauth.kakao.com/oauth/token -d 'grant_type=authorization_code' -d 'client_id=".$this->key_restapi."' -d 'redirect_uri=".$this->redirect_uri."' -d 'code=".$get_data['code']."' -d 'client_secret=".$this->client_secret."'";
		$shell_string = "curl -v -X POST https://kauth.kakao.com/oauth/token -d 'grant_type=authorization_code' -d 'client_id=".$this->key_restapi."' -d 'redirect_uri=".$this->redirect_uri."' -d 'code=".$code."' -d 'client_secret=".$this->client_secret."'";
		...
		if (@!$token_json['access_token']) {
			...
			// [GET] 로그인 회원 정보 가져오기
			$shell_string = "curl -v -X GET https://kapi.kakao.com/v2/user/me -H 'Authorization: Bearer ".$token_json['access_token']."'";
			$user_info = shell_exec($shell_string);
