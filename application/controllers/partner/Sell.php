...
class Sell extends CI_Controller
{
    ...
    public function index($slug)
    {
        ...
        $this->render_layout($data, $this->partner['name']);
    }
    ...
    public function save_devices($slug)
    {
        ...
        $this->session->set_userdata($this->session_key('total_price_text'), '₩' . number_format($computed_total));
    ...
    public function submit($slug)
    {
        ...
        $this->form_validation->set_rules('phone', '연락처', 'required|trim|callback_valid_phone|max_length[20]');
        ...
        if (!$request_id) {
            show_error('수거 요청 저장 중 오류가 발생했습니다.');
        }

        $this->clear_session();
        $this->session->set_flashdata($this->flash_key('complete_message'), '수거 요청이 정상적으로 접수되었습니다.');

        redirect('/' . $this->base_path());
    }

    protected function render_pickup($data)
    {
        $data['buyback_base_path'] = $this->base_path();
        $data['viewPage'] = 'sell/pickup_view';

        $this->render_layout($data, $this->partner['name'] . ' 수거 요청');
    }
    ...
    protected function render_layout($data, $title = '')
    {
        ...
        $data['arr_meta'] = (object) array(
            'title' => $title !== '' ? $title : $this->partner['name'],
        );
