...
class Auth extends CI_Controller
{
	function __construct()
	{
		...
		load_css('<link rel="stylesheet" href="'.CSS_DIR.'/page_auth.css?v=260326" />');
	...
	function register()
	{
		...
		*/
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			if ($use_username) {
				$this->form_validation->set_rules('username', '아이디', 'trim|required|xss_clean|min_length['.$this->config->item('username_min_length', 'tank_auth').']|max_length['.$this->config->item('username_max_length', 'tank_auth').']|alpha_dash|is_unique[users.username]|is_unique[users_admin.username]');
			}

			$use_nickname = $this->config->item('use_nickname', 'tank_auth');
			if ($use_nickname) {
				$this->form_validation->set_rules('nickname', '이름', 'trim|required|xss_clean|min_length['.$this->config->item('nickname_min_length', 'tank_auth').']|max_length['.$this->config->item('nickname_max_length', 'tank_auth').']');
			}
			...
			$this->form_validation->set_rules('confirm_password', '비밀번호 재확인', 'trim|required|xss_clean|matches[password]');
				...
				if (!is_null($data = $this->tank_auth->create_user(
					...
					if ($email_activation) {									// send "activate" email
						...
						*/
						...
						$this->_show_message($user_type . $this->lang->line('auth_message_registration_completed_2').' <div style="padding-top:15px;">'.anchor('/auth/login/', '<button type="button" class="btn btn-dark">'.$user_type.'회원 로그인 바로 가기</button>').'</div>');
