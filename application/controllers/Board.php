...
class Board extends CI_Controller
{
	function __construct()
	{
		...
		// 생성된 게시판 목록 가져오기
		// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		$sql_select = 'bconf.idx, bconf.gr_code, bconf.bo_code, bconf.bo_title, bgrp.gr_title';
		...
		// 게시판 링크 코드 앞쪽
		$this->bbs_url_pre = base_url() . 'board/';
		//$this->bbs_url_pre = $this->arr_seg[1].'/'.$this->arr_seg[2].'/'.$this->arr_seg[3];
	...
	// 게시글 목록, 뷰, 쓰기 및 수정, 삭제
	//function bbs($bo_code=FALSE, $bo_style=FALSE, $wr_idx=FALSE, $page_ttl='page', $page=1)

	//function _remap($bo_code=FALSE) //, $bo_style=FALSE, $wr_idx=FALSE, $page_ttl='page', $page=1)
	//{

	function _remap($bo_code=FALSE)
	{

		if(! $bo_code OR 'index' === $bo_code)
			alert('잘못된 경로로 접속하셨습니다.!!!');

		$bo_style = $this->uri->segment(3, 'lists');
		$wr_idx = $this->uri->segment(4, FALSE);
		$page_ttl = $this->uri->segment(5, 'page');
		$page = $this->uri->segment(6, 1);

		$this->bbs_code_url = $this->bbs_url_pre . $bo_code;

		//$this->bbs_code_url = base_url().$this->bbs_url_pre;
		...
		switch ($bo_style) {
			case 'lists':
				...
				$this->_bbs_lists($bo_code,$page_ttl,$page);
			break;
			case 'request':
				if (! $this->tank_auth->is_admin()) :
					//alert('권한이 없습니다.');
	...
	// 게시글 목록
	function _bbs_lists($bo_code,$page_code=FALSE)
	{
					...
					if($row->opt_secret > 0){
						if(! $this->tank_auth->is_logged_in()  &&  $row->user_idx < 1) { // 비로그인 && 비로그인작성시 user_idx 값은 0
							...
							if (! $this->session->userdata($ss_password_name)) {
								...
								//$password_popup = ' onclick="set_action(\''.$bo_code.'\','.$row->wr_idx.','.$page.'); return false;" data-bs-toggle="modal" data-bs-target="#passModal"  ';
								$password_popup = ' onclick="set_action(\''.$page_code.'\',\''.$bo_code.'\','.$row->wr_idx.','.$page.'); return false;" data-bs-toggle="modal" data-bs-target="#passModal"  ';
							}
							else {
								// 비번 입력하고 패스~
								$this->session->set_userdata($ss_password_name, FALSE);
	...
	function _bbs_detail($bo_code,$wr_idx=FALSE,$page_ttl='page',$page=1)
	{
			...
			// 이전글
				$sql_where_prev = array('ORD <'=>$row->ORD, 'del_yn'=>'N');; // wr_datetime
				if(! $this->tank_auth->is_admin()) {
					$sql_where_prev['APP'] = 'Y';
					$sql_where_prev['VIEW'] = 'Y';
				}
				$sql_order_by_prev = 'ORD DESC, wr_datetime DESC, wr_reply ASC';
				$limit_prev=1;
			...
			// 다음글
				$sql_where_next = array('ORD >'=>$row->ORD, 'del_yn'=>'N');; // wr_datetime
				if(! $this->tank_auth->is_admin()) {
					$sql_where_next['APP'] = 'Y';
					$sql_where_next['VIEW'] = 'Y';
				}
				$sql_order_by_next = 'ORD DESC, wr_datetime DESC, wr_reply ASC';

				$limit_next=1;

				$arr = array('sql_select' =>'*','sql_from' =>$sql_from,'sql_where' =>$sql_where_next,'sql_order_by'=>$sql_order_by_next,'limit'=>$limit_next);
				...
				/*
				echo $this->db->last_query();
				// SELECT * FROM `bbs_newsncase` WHERE `ORD` > '3' AND `opt_notice` < 1 AND `del_yn` = 'N' ORDER BY `ORD` DESC, `wr_datetime` DESC, `wr_reply` ASC LIMIT 1sss
				echo '<br />';
				print_r($row_next);
				exit;
				*/
			...
			*/
			...
			$sql_from = $bo_table;
			$sql_where = array('wr_idx >' => $wr_idx);
			$sql_order_by = 'ORD DESC, wr_datetime DESC, wr_reply ASC';
			$limit = 1; //'20';
	...
	function _bbs_write($bo_code,$wr_idx=FALSE,$page_ttl='page',$page=1,$mode=FALSE)
	{
			...
			*/
			...
			if( $this->input->post('submit') ) {
					...
					//$this->form_validation->set_rules('wr_subject', '제목', 'trim|required');
					...
					if('goodshare' == $bo_code ) 
					{
						...
						$this->form_validation->set_rules('wr_subject', '신청사유', 'trim|required');
						$this->form_validation->set_rules('wr_content', '내용', 'trim|required');
						$this->form_validation->set_rules('opt_secret', '비밀글', '');
						if ($this->tank_auth->is_admin()) {
							$this->form_validation->set_rules('opt_notice', '공지글', '');
						...
						$this->form_validation->set_rules('gubun', '구분', 'trim|required|xss_clean');

						$this->form_validation->set_rules('req_name', '신청자명', 'trim|required');
						...
						$this->form_validation->set_rules('bnf_name', '수혜자명', 'trim|required');
						$this->form_validation->set_rules('bnf_target', '수혜대상', 'trim|required');
						...
						//$this->form_validation->set_rules('addfld_1', '업체명', 'trim|required');
						//$this->form_validation->set_rules('addfld_6', '신청과목', 'trim|required');
					}
					else {
					...
					if ($this->form_validation->run()) {	// validation ok
						...
						//print_r($this->input->post('addfld_5'));
						//exit;
						...
						if (!is_null($data = $this->board_lib->write($bo_code,$wr_idx,$mode,$this->bbs_cf))) {		// success
							...
							if( $this->bbs_cf->bo_use_upload )
							{
								...
								*/
								...
								$cnt_succ_upload = $this->upload_lib->multi_upload_file('attach_file_form[]', url_code('board/'. $bo_code .'/files','e'), $bo_code, $wr_idx, '', $max_upload_size );
							...
							*/
							...
							elseif('write' === $mode) {
								if('form' == $bo_code) {
									sess_message('문의가 접수가 완료되었습니다.');
									alert('문의가 접수가 완료되었습니다.',$redirect_url);
								}
								elseif('goodshare' == $bo_code) {
									sess_message('나눔 신청이 완료되었습니다.');
									alert('나눔 신청이 완료되었습니다.',$redirect_url);
								}
								elseif('reserve' == $bo_code) { // 상담예약
									...
									sess_message('예약 접수가 완료되었습니다.');
									alert('예약 접수가 완료되었습니다.',$redirect_url);
								}
								else {
	...
	// 비회원 비밀글
	function _bbs_password($bo_code,$wr_idx=FALSE,$page_ttl='page',$page=1)
	{
			...
			if( $this->input->post('submit') ) {
					...
					if ($this->form_validation->run()) {	// validation ok
						...
						$arr = array(
								'sql_select'     => '*',
								'sql_from'       => $bo_table,
								'sql_where'      => array('wr_idx'=>$wr_idx, 'wr_password'=>$password)
						);
						$row = $this->basic_model->arr_get_row($arr);

						//if( isset($row->wr_password)  &&  '' !== $row->wr_password  &&  $row->wr_password === $password ){
						if( isset($row->wr_password)){
							// pass

							// 비회원 비밀글
							$ss_password_name = 'ss_bbs_password';
							$this->session->set_userdata($ss_password_name, time());
							redirect( $this->bbs_code_url .'/detail/'.$wr_idx.'/'.$page_ttl.'/'.$page );

						}
						else {
								//sess_message('비밀번호가 일치하지 않습니다..');
								//alert('비밀번호가 일치하지 않습니다..',$this->uri->uri_string());
			...
			$this->data = array(
				...
				'last_loc'      => $last_loc,
				'breadcrumb'    => array('게시판','게시글 관리',$last_loc),
				//'viewPage'      => '_board/skin/password_view'
				'viewPage'      => 'board/password_view'

				//'viewPage'      => '_board/skin/'. $this->bbs_cf->bo_type .'_password_view'
				//'viewPage'      => '_board/skin/'. $this->bbs_cf->bo_type .'_write'. (IS_MOBILE ? '_mobile' : '') .'_view'
			);
	 ...
	 * @return	void
	 */
	function _send_email($type, $email, &$data, $subject=FALSE)
	{
		...
		//$this->email->attach('/home/canta2021.soi9works.com/public_html/assets/images/email/logo.png','inline');
		//$this->email->attach('/home/canta2021.soi9works.com/public_html/assets/images/email/img_00.jpg','inline');
		//$this->email->attach('/home/canta2021.soi9works.com/public_html/assets/images/email/cost_cell.jpg','inline');

		//$data['cid_logo'] = 'logo.png';
		/*
		$data['cid_img_00'] = 'img_00.jpg';
		$data['cid_cost_cell'] = 'cost_cell.jpg';
		*/

		$mail_subject = ($subject && '' != $subject) ? $subject : sprintf($this->lang->line('auth_message_'.$type), $this->config->item('website_name', 'tank_auth'));
