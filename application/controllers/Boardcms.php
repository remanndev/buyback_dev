...
class Boardcms extends CI_Controller
{
	...
	*/
	function _remap($page_code=FALSE,$arr=FALSE)
	{
		...
		// 메뉴 정보 가져오기
		// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		//$page_code = $this->arr_seg[2];
		$cms_arr = array('sql_select' => '*','sql_from' => 'mng_cms','sql_where' => array('page_code'=>$page_code, 'page_type'=>'boardpage'));
		$cms_row = $this->basic_model->arr_get_row($cms_arr);
		if((! isset($cms_row->page_code) OR $cms_row->del_datetime !== NULL)) {
			alert('이미 삭제되었거나, 존재하지 않는 페이지입니다.', base_url());
		}

		$bo_code = isset($cms_row->page_flag) ? $cms_row->page_flag : false;
		...
		$bo_style = isset($arr[1]) ? $arr[1] : 'lists';
		if(! $bo_style)
		...
		//$wr_idx = $this->uri->segment(5, FALSE);
		$wr_idx = isset($arr[2]) ? $arr[2] : FALSE;

		//$page_ttl = $this->uri->segment(6, 'page');
		$page_ttl = isset($arr[3]) ? $arr[3] : 'page';

		//$page = $this->uri->segment(7, 1);
		$page = isset($arr[4]) ? $arr[4] : 1;
		...
		switch ($bo_style) {
			case 'lists':
				...
				$this->_bbs_lists($bo_code,$page_code);
			break;
			case 'request':
				if (! $this->tank_auth->is_admin()) :
					//alert('권한이 없습니다.');
	...
	function _bbs_lists($bo_code,$page_code=FALSE)
	{
			...
			foreach($result['qry'] as $i => $row) {
				...
				$썸네일이미지_src = '';
					...
					if($row->wr_idx)
					{
						...
						*/
						...
						if(! empty($file_row_bnr)) {
							...
							$raw_img_src = DATA_DIR.'/'.$file_row_bnr->file_dir.'/'.$file_row_bnr->file_name;
							$raw_img_path = DATA_PATH.'/'.$file_row_bnr->file_dir.'/'.$file_row_bnr->file_name;
					...
					$result['qry'][$i]->raw_img_path = $raw_img_path; // 기존 디비에 등록된 이미지

					$result['qry'][$i]->resize_thumb_src = $썸네일이미지_src;
					$result['qry'][$i]->resize_thumb_path = $썸네일이미지_path;
		...
		// load css, js  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
			...
			$this->data = array(
				...
				'total_count' => $total_count, // 카테고리 등 검색 빼고 전체
	...
	function _bbs_write($bo_code,$wr_idx=FALSE,$page_ttl='page',$page=1,$mode=FALSE)
	{
			...
			// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
			// [2022-11-18] 함께하는기관(bbs_orgs) 게시판은 한 ID 당 하나의 게시물만 작성 가능하도록 내용 추가
			if('bbs_orgs' == $bo_table && 'write' == $mode) {
				...
				if ($this->tank_auth->is_admin()) {
					...
					if( isset($row->wr_idx)  &&  $row->wr_idx > 0) {
						$redirect_url = base_url().'B/PEOPLE/함께-하는-기관/detail/'.$row->wr_idx;
						alert('이미 「함께 하는 기관」 소개를 작성하셨습니다.\n회원님의 기관 소개 페이지로 이동합니다.',$redirect_url);
					}
			...
			*/
			...
			if( $this->input->post('submit') ) {
					...
					if('orgs' == $bo_code ) 
					{
						$this->form_validation->set_rules('wr_content', '기관 소개', 'trim|required|xss_clean');
						$this->form_validation->set_rules('wr_subject', '기관명', 'trim|required|xss_clean');
