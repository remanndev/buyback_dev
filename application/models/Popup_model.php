...
class Popup_model extends CI_Model {
	...
	function list_result($sst, $sod, $sfl, $stx, $limit, $offset) {
		...
		$result['total_cnt'] = $this->db->count_all_results('mng_popup');
		...
		$qry = $this->db->get('mng_popup', $limit, $offset);
	...
	function get_popup($pu_id, $fields='*') {
		...
		return $this->db->select($fields)->get_where('mng_popup', array('pu_id' => $pu_id))->row_array();
	}

	function record($w='',$pu_id=FALSE) {
		$sql = array(
			'pu_name' => $this->input->post('pu_name'),
			'pu_file' => ($this->input->post('pu_file')) ? $this->input->post('pu_file') : '',
			...
			'pu_sdate' => $this->input->post('sdatetime'),
			'pu_edate' => $this->input->post('edatetime'),
		...
		if ($w == '') {
			...
			$this->db->insert('mng_popup', $sql);
			return $this->db->insert_id();
		}
		else {
			if( ! $pu_id)
				$pu_id = $this->input->post('pu_id');
			$this->db->update('mng_popup', $sql, array('pu_id' => $pu_id));
			return $pu_id;
		}
	}
	...
	function list_update($pu_ids, $pu_names, $pu_uses) {
		...
		$this->db->update_batch('mng_popup', $batch, 'pu_id');
	}

	function delete($pu_ids) {
		$this->db->where_in('pu_id', $pu_ids);
		$this->db->delete('mng_popup');
	}
    ...
    function output() {
		//if ($this->config->item('cf_use_popup')) {
			...
			return $this->db->select('pu_id,pu_file,pu_content,pu_type,pu_width,pu_height,pu_x,pu_y')->get_where('mng_popup', array(
				//'host' => HTTP_HOST,
...
/* End of file admin_popup_model.php */
/* Location: ./application/models/admin_popup_model.php */